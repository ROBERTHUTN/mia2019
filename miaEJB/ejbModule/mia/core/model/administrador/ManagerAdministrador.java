...
public class ManagerAdministrador {
	...
	// M√©todo que me devuelve la Lista de ROLES
...
// M√©todo que me devuelve la Lista de ROLES
	...
	// M√©todo que me devuelve la Lista de PAISES_ESTADO
...
//M√©todo que ingresa al usuario
	public void ingresarUsuario(Usuario usuario, int id_rol) throws Exception {
		...
		if (correoE) {
			...
			throw new Exception("Error ya existe un usario registrado con ese correo electr√≥nico");
		}

		boolean ceula = existeCedulaUsuario(usuario.getCi());
		if (ceula) {
			throw new Exception("Error ya existe un usario registrado con ese n√∫mero de c√©dula");
		}
	...
	public void ingresarPais(PaisEstado paisEstado, int id_padre_pais) throws Exception {
		if (paisEstado == null) {
			throw new Exception("No ha ingresado datos en el Pa√≠s");
		}
		...
		if (paisE) {
			...
			throw new Exception("Error ya existe un pa√≠s o estado registrado con ese nombre");
		}
...
//M√©todo que me ingresa la ficha personal
	public void ingresarFichaPersonal(FichaPersonal fichaPersonal, int id_religion, int id_etnia, int id_pais,
		...
		if (id_religion == 0) {
			throw new Exception("Error al seleccionar la religiÛn");
		}
		if (id_etnia == 0) {
			throw new Exception("Error al seleccionar la etnia");
		}
		if (id_pais == 0) {
			throw new Exception("Error al seleccionar el paÌs");
		}
		if (estado == 0) {
	...
	public void ingresarReligion(Religion reli) throws Exception {
		if (reli == null) {
			throw new Exception("Ingrese los datos de la religi√≥n");
		}
		boolean existeReligion = existeReligion(reli.getDescripcion());
		if (existeReligion) {
			throw new Exception("La religi√≥n  " + reli.getDescripcion() + " ya existe");
		}
	...
	public void ingresarEtnia(Etnia etnia) throws Exception {
		if (etnia == null) {
			throw new Exception("Ingrese los datos de la Etn√≠a");
		}
		boolean existeEtnia = existeEtnia(etnia.getDescripcion());
		if (existeEtnia) {
			throw new Exception("La Etn√≠a  " + etnia.getDescripcion() + " ya existe");
		}
	...
	public void editarUsuario(Usuario usuarioA, int id_rol) throws Exception {
		...
		if (cedulaRepetida) {
			throw new Exception("La cÈdula ya est· registrada con otro usuario");	
		}
		if (correoRepetido) {
	...
	public void editarReligion(Religion relA) throws Exception {
		...
		if (relN == null) {
			throw new Exception("Error al cargar la religiÛn");
		}
		if (!relA.getDescripcion().equals(relN.getDescripcion())) {
			...
			if (existeNombreReligion) {
				throw new Exception("Ya existe la religi√≥n con el nombre " + relA.getDescripcion());
			}
		}
	...
	public void eliminarRol(int id_rol) throws Exception {
		...
		if (existeRolUsuario) {
			throw new Exception("El rol est· siendo utilizada en la tabla usuario");
		}
		em.remove(rolN);
	}
	...
	public void eliminarUsuario(long id_usuario) throws Exception {
		...
		if (existeUsuarioRespuesta) {
			throw new Exception("El usuario est· siendo utilizado en la tabla de Respuestas no puede ser eliminado!");
		}
		boolean existeUsuarioReporte = existeUsuarioReporte(id_usuario);
		if (existeUsuarioReporte) {
			throw new Exception("El usuario est· siendo utilizado en la tabla de Reporte no puede ser eliminado!");
		}
		boolean existeUsuarioFichaPersonal = existeUsuarioFichaPersonal(id_usuario);
		if (existeUsuarioFichaPersonal) {
			throw new Exception(
					"El usuario est· siendo utilizado en la tabla de FichaPersonal no puede ser eliminado!");
		}
		em.remove(usuarioN);
	}
	...
	public void eliminarReligion(int id_religion) throws Exception {
		...
		if (reliN == null) {
			throw new Exception("Error al cargar la religi√≥n");
		}
		boolean existeReligionFicha = existeReligionenFichaRegistro(id_religion);
		if (existeReligionFicha) {
			throw new Exception("La religi√≥n est√° siendo utilizada en la tabla ficha de registro");
		}
		em.remove(reliN);
	}
	...
	// MÈtodo que devuelve la OrganizaciÛn
	...
	**/
	public void ingresarUsuarioProyecto( UsuarioProyecto usuarioproyecto ,long id_ficha_fk, long  id_proyecto, int id_organizacion) throws Exception {
		...
		if (id_ficha_fk == 0 || id_proyecto==0 || id_organizacion==0) {
			throw new Exception("Ingrese los datos del proyecto, encargado y organizaciÛn.");
		}
		/**
		boolean existeAreaInteresFicha=existeOrganizacionFicha(id_area, id_ficha_fk);
		if (existeAreaInteresFicha) {
			throw new Exception("Ya se encuentra el ·rea con el usuario");
		}
		*/
	...
	public void eliminarAreaInvestigacion(int id_AreaInvestigacion) throws Exception {
		...
		if (areaInvestigacionN == null) {
			throw new Exception("Error al cargar el ·rea de investigaciÛn");
		}
